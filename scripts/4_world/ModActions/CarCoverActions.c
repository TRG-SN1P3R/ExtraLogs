...
modded class ActionAddCarCover: ActionCoverUncoverCar {
    override void OnFinishProgressServer(ActionData action_data) {
        ...
        if(m_LogConfig.ModdedActions.ShowCarCoverActions==0) return;//Do we want to see this?
        SendToCFTools(action_data.m_Player, "hands", action_data.m_Target.GetObject().GetType(), "added a car cover to");
    }
}

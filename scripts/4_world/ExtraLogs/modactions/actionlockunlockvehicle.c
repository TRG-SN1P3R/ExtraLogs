...
modded class ActionUnlockVehicle: ActionLockUnlockVehicle {
	override void OnFinishProgressServer(ActionData action_data) {	
        ...
        SendToCFTools(action_data.m_Player, action_data.m_MainItem.GetType(), action_data.m_Target.GetObject().GetType(), "unlocked");
        }
        else{
...
modded class ActionUnlockVehicleInside: ActionLockUnlockVehicle {
	override void OnFinishProgressServer(ActionData action_data) {	
        ...
        if (m_LogConfig.ServerConfig.ShowCarActions==0) return; //Do we want to see this?
		if (m_LogConfig.ServerConfig.SimpleLogs==1){//Show only name
        SendToCFTools(action_data.m_Player, action_data.m_MainItem.GetType(), action_data.m_Target.GetObject().GetType(), "unlocked from inside");
        }
        else{
        SendToCFTools(action_data.m_Player, action_data.m_MainItem.GetType(), string.Format("%1",action_data.m_Target.GetObject()), "unlocked from inside");            
        }
...
modded class ActionLockVehicle: ActionLockUnlockVehicle {
	override void OnFinishProgressServer(ActionData action_data) {	
        ...
        if (m_LogConfig.ServerConfig.ShowCarActions==0) return; //Do we want to see this?
		if (m_LogConfig.ServerConfig.SimpleLogs==1){//Show only name
        SendToCFTools(action_data.m_Player, action_data.m_MainItem.GetType(), action_data.m_Target.GetObject().GetType(), "locked");
        }
        else{
